...
- 환경 진단: `uv run <프로젝트루트>/{{AGENT_DIR}}/scripts/setup_check.py <프로젝트루트>`
...
`<프로젝트루트>/{{AGENT_DIR}}/scripts/setup_check.py`가 없으면:
→ "플러그인 파일이 설치되지 않았습니다. `npx @koreainvestment/kis-quant-plugin init --agent <agent> --force`를 실행하세요."
